...
The feature is entirely opt-in: it's compiled in via the `traffic` Cargo feature (`cargo build --features traffic`, already included in the published Docker images) and only runs when `TRAFFIC_ENABLED=true`. A build or config without it costs nothing at runtime — the query endpoints simply don't exist (feature off) or 404 (feature on, but disabled).
